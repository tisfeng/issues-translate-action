...
本计划最初把提交列为非目标；随后用户明确调用 `git-commit`，本任务代码、测试、运行时
产物和文档已在本地提交 `67da8562a48b2aa72016bd6499d06e261291c449`，且未 push。该次显式
交付还纳入了任务开始前已有的 v2.8.3 release 记录；这是当时 `git add .` 行为造成的事实，
不是当前规则允许的自动交付范围。现行规则要求自动提交仅精确暂存 Agent-owned paths。
