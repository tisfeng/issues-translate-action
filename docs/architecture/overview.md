...
3. `getTranslationTarget()` 先调用 `getLanguageDetectionText()` 生成检测副本：保留 Markdown
   可见文字，忽略 URL、代码、链接目标和 commit SHA 等技术噪声。若 Latin 技术词可能压过
   有充分证据的中日韩文字，Unicode script 规则优先返回对应检测代码；Han-only 的中日歧义
   仍回退给 `franc-min`。不是 `PRIMARY_LANGUAGE` 的内容翻译为第一语言；第一语言内容在
   配置 `SECONDARY_LANGUAGE` 时翻译为第二语言，否则跳过。无法判断时仍尝试翻译为第一语言。
...
6. 翻译结果用于更新标题或生成机器人回复。仅 NFC 或换行编码不同的译文视为无变化，不会
   写回 GitHub；自定义 token 会查询或比对机器人登录名，机器人自身产生的事件在翻译请求前跳过。
...
- Markdown 原文用于翻译和回写。检测副本仅移除技术噪声，避免图片/链接地址、代码或 SHA
  将可见正文误判为另一种语言；清理后的文本绝不用于翻译或回写。
