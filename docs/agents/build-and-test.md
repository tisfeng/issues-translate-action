...
本文件规定变更本 Action 时的本地验证。测试设计见 [`testing.md`](testing.md)，GitHub 事件、
token 与真实外部副作用见 [`github-action.md`](github-action.md)。
...
- `npm run all` 会执行写入式 `npm run format`，且不运行 test 或 lint，不能作为最终验证入口。
...
- 修改 `src/` 或运行时 dependency 后，必须运行 `npm run package` 并提交预期 `dist/` 产物；
  不要手工编辑 `dist/index.js`、source map 或 license 文件。
- 测试必须覆盖行为而不是仅覆盖内部实现细节；变更事件、评论目标、标题更新、语言判断或
  翻译输出时，补充对应回归测试。
- 文档或治理规则改动不需要运行 npm build、test 或 package，除非同一任务还改动了运行时路径。
